...
namespace Age_Of_Nothing.Sprites.Resources
{
    ...
    public class Forest : Resource
    {
        ...
        private Forest(Point center, int forestPatchIndex, Controller parent)
            : base(center, _quantity, parent)
        {
        ...
        /// <returns></returns>
        public static IEnumerable<Forest> GenerateForestPatch(Rect patchSurface, Controller parent, int forestPatchIndex)
        {
            ...
            for (var i = patchSurface.X; i < patchSurface.BottomRight.X; i += size.Width)
            {
                ...
                {
                    yield return new Forest(new Point(i + size.Width / 2, j + size.Height / 2), forestPatchIndex, parent);
                }
            }
        }
    }
